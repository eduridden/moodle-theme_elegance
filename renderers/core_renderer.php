...
class theme_elegance_core_renderer extends core_renderer {
    ...
    protected function render_user_menu(custom_menu $menu) {
        ...
        if ($addmessagemenu) {
            ...
            $messagemenu = $menu->add('<i class="fa fa-comments"></i>' . $messagecount . ' ' . get_string('messages', 'message'), new moodle_url('#'),  get_string('messages', 'message'), 9999);
        ...
        if ($addusermenu) {
            if (isloggedin()) {
                ...
                $usermenu->add('<i class="fa fa-cog"></i>' . get_string('editmyprofile'), new moodle_url('/user/edit.php',
                array('id'=>$USER->id)),
                get_string('editmyprofile'));
                
                $usermenu->add('<i class="fa fa-calendar"></i>' . get_string('calendar','calendar'), new moodle_url('/calendar/view.php?view=month',
                array('sesskey'=>sesskey(),'alt'=>'calendar')),
    ...
    protected function get_user_messages() {
        ...
        foreach ($newmessages as $message) {
            $messagelist[] = $this->elegance_process_message($message, 'new');
        }

        foreach ($readmessages as $message) {
            $messagelist[] = $this->elegance_process_message($message, 'old');
        }
        return $messagelist;

    }

    protected function elegance_process_message($message, $state) {
        global $DB;
